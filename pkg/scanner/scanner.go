...
func (s *Scanner) Scan(in ScanInput) {
	...
	var restore_tracker_commits tracker.KeyDataMap
	var restore_tracker_files tracker.KeyDataMap
	...
	if cpoint != nil {
		...
		restore_tracker_commits = cpoint.TrackerCommitsData
		restore_tracker_files = cpoint.TrackerFilesData
	...
	go s.trackScanProgress(in.RepoID, chan_scan_done, chan_quit)
...
func (s *Scanner) checkpointScan(
	...
	scan_repo, err := s.getScanRepository(repo_id)
	if err != nil {
	...
	setNewCheckpoint := func() (e error) {
		...
		e = CheckpointSet(
			...
			commit_id,
			NewCheckpoint(
				scan_repo.TrackerCommits.GetKeysData(),
				scan_repo.TrackerFiles.GetKeysData(),
...
func (s *Scanner) scanRepository(
	...
	restore_tracker_commits tracker.KeyDataMap,
	restore_tracker_files tracker.KeyDataMap,
) {
	...
	// restore tracker state for commits and files if necessary
	if len(restore_tracker_commits) > 0 {
		scan_repo.TrackerCommits.Restore(restore_tracker_commits)
	}
	if len(restore_tracker_files) > 0 {
		scan_repo.TrackerFiles.Restore(restore_tracker_files)
	}
	...
	if scan_err := scan_repo.Scan(); scan_err != nil {
...
func (s *Scanner) trackScanProgress(
	...
	trackScanCounts := func() (done bool) {
		...
		if err := CheckpointDelete(s.ctx, s.git_config.WorkDir, scan_repo.ID, ""); err != nil {
